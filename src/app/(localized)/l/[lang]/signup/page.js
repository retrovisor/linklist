...
export default async function SignupPage({ params: { lang }, searchParams }) {
  console.log('SignupPage function started');
  try {
    const session = await getServerSession(authOptions);
    console.log('Session:', session);

    const translations = await fetchTranslations(lang, 'signup');
    
    return <SignupPageClient session={session} searchParams={searchParams} translations={{ lang, ...translations }} />;
  } catch (error) {
